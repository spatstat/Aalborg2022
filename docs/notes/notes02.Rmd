...
Are the points clustering more than expected only from dependence on `grad`?
...
Above the 99 simulations from the fitted Poisson model are generated. 
Then `Linhom()` for these and the data using leave-one-out KDE for `lambda`.
...
There is still another worry about the composite hypothesis, where we have used data to estimate the parameters of the model.
